...
$( document ).ready(function() {
    ...
    uibuilder.onChange('msg', function(newVal){
        ...
        switch(newVal.topic){
            case "ptinfo":
                // console.log(newVal);
                var name=(newVal.payload.ptinfo.PAPMI_NAME[0]) + ' ' + (newVal.payload.ptinfo.PAPMI_NAME2[0]);
                $('#name').text(name);
                $('#ptName').text(name);
                $('#age').text(newVal.payload.ptinfo.AGE[0]);
                $('#dob').text(newVal.payload.ptinfo.PAPMI_DOB[0]);
                drawChart(newVal);
...
function sendHN(me){
    console.log($('#hn').val());
    HN=$('#hn').val();
    uibuilder.send({'topic':'HN','payload':HN});
}

function hnChange(e){
    console.log(e.keyCode);
    if(e.keyCode==13){sendHN(0)}
}

function barChart() {
    var _chart = {};
// debugger;
    var _width = 450, _height = 250,
    ...
    _chart.render = function () {
        d3.select("#svg01").html("");
        if (!_svg) {
            _svg = d3.select("#svg01").append("svg")
                .attr("height", _height)
                .attr("width", _width);
    ...
    function renderAxes(svg) {
        var axisLength=_width-_margins.left-_margins.right;
    ...
    function renderBars() {
        ...
        bars.enter()
            ...
            .attr("height", function (d) {
                // console.log(yStart(),_y(d.y),_y(d.yd),_y(d.y)-_y(d.yd));
                return _y(d.yd)-_y(d.y);
            })
...
function drawChart(scope) {
    // debugger;
    var numberOfDataPoint = scope.obs.length,
        // var data = [];
        data = scope.obs;
    data = d3.range(data.length).map(function (i) {
        // debugger;
        return {x: new Date(data[i].AdmDate), y: data[i].BPSystolic, yd: data[i].BPDiastolic};
    });

    var chart = barChart()
        .x(d3.scaleTime().domain([new Date(scope.obs[0].AdmDate), new Date(scope.obs[data.length - 1].AdmDate)]))   // set x axis
        .y(d3.scaleLinear().domain([0, 200])); // set y axis
